...
def vary_corr(start, end, dir):
    ...
    parameter_list = np.array([0, 0.2, 0.4, 0.6])
    pval_dict = {}
    ...
    for rho in parameter_list:
        for i in range(start, end):
            ...
            print(rho, i, "th simulation done")
            print("seed:", i + 1000)
            ...
            if not ds_rank_def and not noselection_ds:
                ...
                pval_dict[rho]['Data Splitting'] += (p_values_ds)
                oper_char["power"].append(calculate_power(p_values_ds, targets_ds, 0.1))
                update_targets(dict=target_dict,
                               true_inter_list=None,
                               targets=targets_ds, parameter=rho,
            ...
            if not noselection_MLE:
                ...
                pval_dict[rho]['MLE'] += (p_values_MLE)
                oper_char["power"].append(calculate_power(p_values_MLE,
                                                          targets_MLE,
                                                          0.1))
                update_targets(dict=target_dict,
                               true_inter_list=None,
                               targets=targets_MLE, parameter=rho,
